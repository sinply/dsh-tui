...
The `pnpm-workspace.yaml` pins the `@deepseek-ai/dsh-*` peer packages to the exact `0.1.2-alpha.2` versions — the registry only publishes prereleases, which a bare `>=0.0.1` range cannot match.
...
- dsh baseline lifted to 0.1.2-alpha.2: user-questions answerers compose on the `user-questions/request` waterfall, title cold reads move to the session query engine, and todo types come from `dsh-tool-todo`/`dsh-util-values`
