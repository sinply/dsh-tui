...
const TIMING_BUCKETS = ['ttft', 'thinking', 'responding', 'tools'];
...
function enterTimingBucket(state, bucket, at) {
    if (state.active?.bucket === bucket)
        return;
    closeTimingBucket(state, at);
    if (bucket !== undefined)
        state.active = { bucket, since: at };
}
function advanceStepTiming(state, event) {
    if (event.type === 'assistant/chunk') {
        const chunk = event.data.chunk;
        if (state.active?.bucket === 'ttft')
            enterTimingBucket(state, undefined, event.time);
        ...
        for (; this.scanned < events.length; this.scanned += 1) {
            ...
            if (event.type === 'step/start') {
                ...
                if (!this.steps.has(key))
                    this.steps.set(key, { ...timingState(event.time), closed: false });
            }
 ...
 */
export const TIMING_BUCKET_GLYPHS = {
    ttft: '◍',
    thinking: '✻',
    responding: '●',
    tools: '⚙',
 ...
 */
export function openStepPhase(events) {
    let startIndex = -1;
    let start;
    for (let index = events.length - 1; index >= 0; index -= 1) {
        const event = events[index];
 ...
 */
export function runningPhaseGlyph(events, running, compacting) {
    if (running) {
 ...
 */
export function pulseLevel(nowMs) {
 ...
 */
export function fadeGlyph(glyph, palette, colorEnabled, truecolor, opacity, visible) {
 ...
 */
export function formatStatusDuration(elapsedMs) {
 ...
 */
export function formatTimingTotals(totals, includeModelWait = false) {
 ...
 */
export function formatQueuedStatus(queued) {
 ...
 */
export function formatCompletionTime(time) {
