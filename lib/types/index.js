 ...
 */
import { CombinedAutocompleteProvider, Container, Key, Spacer, Text, TUI, ProcessTerminal, matchesKey, visibleWidth, } from '@earendil-works/pi-tui';
...
import { TuiExtensionServiceImpl, TuiOverlayManager, } from "./extension/overlay-manager.js";
import { parseTuiPromptTemplate, renderTuiPromptTemplate, } from "./prompt.js";
import { displayInlineText, displayText } from "./components/text.js";
import { brandText, createPalette, createVscodePalette, markdownTheme, renderPalette, selectTheme } from "./components/theme.js";
import { contentText, parseArguments } from "./components/content.js";
import { cacheHitRate, formatTokens, recordEventUsage, sessionTokens, } from "./chat/tokens.js";
import { fadeGlyph, formatQueuedStatus, formatStatusDuration, openStepPhase, openTurn, pulseLevel, runningPhaseGlyph, STATUS_ANIMATION_INTERVAL_MS, STATUS_FADE_MS, StepTimingTracker, TIMING_BUCKET_GLYPHS, } from "./chat/timing.js";
import { resolveTuiConfig, } from "./config.js";
import { ContextCardComponent, HeaderComponent, StreamingAssistantComponent, ToolCardComponent, TodoComponent, UserMessageComponent, } from "./components/transcript.js";
import { compactTargetLabel, DetailsDialog, diagnosticMeter, formatDiagnosticCount, formatDiagnosticNumber, formatDiagnosticTime, initialTarget, StatusCardComponent, PromptContextComponent, targetLabel, } from "./components/dialogs.js";
import { parseSkillCommand, renderSkillInvocation, SKILL_COMMAND_PREFIX, } from "./chat/skill-invocation.js";
import { ReferenceAutocompleteProvider } from "./chat/autocomplete.js";
import { BANNER_REVEAL_INTERVAL_MS, formatCwd, gitBranch, HintEditor, isCompactCheckpoint, sessionReferenceCard, transcriptToolCallIds, } from "./chat/helpers.js";
import { createModelController, } from "./chat/model-command.js";
import { createQuestionQueue } from "./chat/questions.js";
import { createResumeController } from "./chat/resume.js";
import { WorkspaceFileSearch } from "./chat/file-autocomplete.js";
export { TuiPromptService } from "./prompt.js";
export { renderSkillInvocation } from "./chat/skill-invocation.js";
export { resolveTuiConfig, TuiConfigSchema, Config, } from "./config.js";
export { DEFAULT_FILE_SEARCH_EXCLUDED_DIRECTORIES, DEFAULT_FILE_SEARCH_MAX_ENTRIES, DEFAULT_FILE_SEARCH_MAX_RESULTS, } from "./chat/file-autocomplete.js";
...
function lastSessionActivityTime(events) {
    ...
    for (const event of events) {
        if (latest === undefined || event.time > latest)
            latest = event.time;
    }
    return latest;
}
 ...
 */
export class TuiExtensionService extends Service {
}
 ...
 */
export function createTuiChat(ctx, config, runtime) {
    const sessionId = SessionId(config.sessionId ?? 'main');
    const agent = ctx.agents.get(sessionId);
    if (agent === undefined)
        throw new Error(`ui-tui: session "${sessionId}" is not running`);
    const resolved = resolveTuiConfig(config);
    // The VSCode-blue 24-bit palette is the shipped look when color is enabled;
    // the adaptive ANSI palette remains the fallback (and the /palette reference).
    const palette = resolved.theme.color && resolved.theme.vscode
        ? createVscodePalette(true)
        : createPalette(resolved.theme.color);
    ...
    let tuiServiceFiber;
    const target = { current: initialTarget(agent), assembled: undefined };
    ...
    const header = new HeaderComponent(agent, () => sessionTitle ?? config.welcome, palette, resolved.theme.color && (resolved.theme.truecolor || resolved.theme.vscode));
    ...
    const updatePromptValues = () => {
        ...
        contextValue.set(contextWindow === undefined ? undefined : `  ${palette.dim(`${Math.min(100, Math.round(ctx.tokenMeter.measure(agent.session).totalTokens / contextWindow * 100))}% context`)}`);
        ...
        const statusGlyph = runningPhaseGlyph(agent.session.events, runningStatus !== undefined, compacting !== undefined);
        ...
        if (runningStatus !== undefined && statusGlyph !== undefined)
            runningStatus.lastGlyph = statusGlyph;
        ...
        const caret = envelope === undefined
            ? palette.dim('>')
            : fadeGlyph(envelope.glyph, palette, resolved.theme.color, resolved.theme.color && resolved.theme.truecolor, envelope.level * pulseLevel(renderTime), envelope.level >= 0.5);
    ...
    const updateTerminalTitle = () => {
        runtime.terminal.setTitle(displayText(sessionTitle === undefined ? resolved.title : `${sessionTitle} — ${resolved.title}`));
    };
    updateTerminalTitle();
    const requestRender = () => {
        if (disposed)
    ...
    // A full-width rule separating transcript blocks (You / Assistant / injected
    // context), keeping each role visually distinct from the next.
    ...
    const extensionTheme = Object.freeze({
        ...
        brand: (value) => resolved.theme.color
    ...
    const beginFadeOut = (glyph) => {
        ...
        const fading = {
            ...
            endedAt: now(),
            timer: setInterval(() => {
                if (now() - fading.endedAt >= STATUS_FADE_MS)
                    clearTurnStatus();
                renderStatus();
    ...
    const setStatus = (status) => {
        ...
        if (status === 'running')
            clearTurnStatus();
        else if (fadeOutGlyph !== undefined)
            beginFadeOut(fadeOutGlyph);
        else
            clearTurnStatus();
    ...
    const renderEvent = (event, options) => {
        switch (event.type) {
            ...
            case 'tool/result': {
                ...
                if (card === undefined) {
                    card = new ToolCardComponent('tool', { value: {}, valid: true }, undefined, resolved.maxToolOutputLines, resolved.maxDiffEditLength, palette, mdTheme);
                    card.setVisibility(toolsVisibility);
                    chat.addChild(card);
                    allToolCards.add(card);
                }
            ...
            case 'turn/end': {
                ...
                switch (reason.kind) {
                    ...
                    default:
                        ...
                        appendNotice(`Turn ended: ${reason.kind}.`, 'warning');
                        break;
                }
                break;
            }
    ...
    const rebuildTranscript = (populateHistory) => {
        ...
        for (const event of agent.session.events) {
            ...
            if (event.type === 'tool/call' && !transcriptCalls.has(event.data.callId))
                continue;
    ...
    const resume = createResumeController({
        ...
        sessionQuery: () => {
            ...
            if (implementation === undefined || implementation.fiber.state >= FIBER_FAILED)
                return undefined;
    ...
    const applyColorScheme = (scheme) => {
        ...
        Object.assign(palette, resolved.theme.color && resolved.theme.vscode
            ? createVscodePalette(true)
            : createPalette(resolved.theme.color, scheme));
    ...
    const setToolsVisibility = (next) => {
        ...
        for (const turn of assistantSteps.keys())
            applyTurnFolding(turn);
    ...
    const toggleReasoning = () => { setReasoning(!showReasoning); };
    ...
    const showDetailsSelector = () => {
        ...
        const session = overlayManager.open({
            create: () => new DetailsDialog(toolsVisibility, showReasoning, palette, 
            // Each Tab applies immediately; one dimension changes per call.
            (selection) => {
                if (selection.showReasoning !== showReasoning)
                    setReasoning(selection.showReasoning);
                if (selection.visibility !== toolsVisibility)
                    setToolsVisibility(selection.visibility);
            }, () => { void session.close(); }),
            options: { width: resolved.detailsDialogWidth, anchor: 'center', margin: 1 },
        });
        detailsOverlay = session;
        void session.closed.then(() => {
            if (detailsOverlay === session)
                detailsOverlay = undefined;
        });
        requestRender();
    };
    ...
    const refreshSkillCommands = (service) => {
        ...
        service.snapshot({ cwd, signal: skillAbort.signal }).then((snapshot) => {
            if (disposed || scan !== skillCommandScan || !snapshot.complete)
                return;
    ...
    const runCommand = (text) => {
        ...
        void ctx.commands.execute(agent, text, [], controller.signal).then((execution) => {
            if (disposed)
                return;
    ...
    const dispatchMessage = (content) => {
        ...
        if (agent.status === 'running') {
            // While the driver is live, the prompt steers the open turn; the
            // durable user/message lands inside it (see the agent inbox contract).
    ...
    const runReload = () => {
        ...
        for (const entry of loader.entries()) {
            if (entry.subtree?.refresh !== undefined)
                refreshes.push(entry.subtree.refresh());
        }
        reloadInFlight = true;
    ...
    editor.onSubmit = (value) => {
        ...
        if (text.startsWith(SKILL_COMMAND_PREFIX)) {
            ...
            if (skillName === '')
                appendNotice('Usage: /skill:<name> [instructions]', 'warning');
            else
                invokeSkill(skillName, instructions);
            return;
        }
        if (value.startsWith('/')) {
            editor.addToHistory(text);
        ...
        editor.addToHistory(text);
        editor.setText('');
        // Session-reference `@` mentions are resolved by the session-reference
        // plugin when the step is proposed (`agent/pre-step`), so no submit-time
        // preparation is needed here.
        dispatchMessage([{ type: 'text', text }]);
    };
    const removeInputListener = ui.addInputListener((data) => {
        ...
        if (matchesKey(data, Key.ctrl('d'))) {
            if (agent.status === 'running')
                appendNotice('Cancel the active turn before exiting.', 'warning');
            else
                requestExit();
            return { consume: true };
        }
    ...
    const disposeSessionEvents = ctx.on('session/event', (session, event) => {
        ...
        if (event.type === 'compaction/end' && event.data.turn === null && compacting !== undefined) {
            ...
            if (runningStatus === undefined && fadeOutGlyph !== undefined)
                beginFadeOut(fadeOutGlyph);
            requestRender();
            return;
        }
    ...
    const disposeDiscarded = ctx.on('agent/inbox/discarded', ({ agent: subject, message }) => {
        if (subject !== agent)
            return;
        if (pendingSteering.delete(message.id))
            refreshStatus();
    });
    ...
    // Sweep reveal of the whole banner: the header lines appear top-to-bottom
    // over ~BANNER_REVEAL_STEPS frames (started after `ui.start()` succeeds).
    ...
    const startBannerReveal = () => {
        if (config.welcome !== undefined)
            return;
        // Five art rows, a blank, and the session detail line.
        const total = 7;
        let shown = 0;
    ...
    if (restoredGoal !== undefined && restoredGoal.phase !== 'complete') {
        appendNotice(`Goal restored (${restoredGoal.phase}) with automatic continuation disarmed. `
            + 'Human confirmation is required; send “继续” or run /goal resume.', 'warning');
    }
    setStatus(agent.status);
    try {
        ui.start();
    ...
    if (config.initialSkill !== undefined)
        invokeSkill(config.initialSkill, '');
    return {
        async dispose() {
            detachListeners();
 ...
 * @param runtime - Terminal and process-exit boundary.
 */
export function mountTui(ctx, config, runtime) {
    const sessionId = SessionId(config.sessionId ?? 'main');
    ...
    const start = (agent) => {
        if (settled || !matchesConfiguredIdentity(agent))
            return;
        settled = true;
        stopWaiting();
        ctx.effect(() => {
 ...
 */
export function disposeRootAndExit(ctx, code, exit = (status) => { process.exit(status); }) {
    let exited = false;
    const exitOnce = () => {
        if (exited)
            return;
    ...
    void ctx.root.fiber.dispose().then(() => { clearTimeout(timeout); exitOnce(); }, () => { clearTimeout(timeout); exitOnce(); });
}
...
export function apply(ctx, config) {
    ...
    mountTui(ctx, Object.assign({}, config, { theme: Object.assign({}, config.theme, { truecolor }) }, initialSkill === undefined ? {} : { initialSkill }), {
