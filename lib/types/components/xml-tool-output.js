...
function parseXml(source, display) {
    ...
    const reject = () => { state.invalid = true; };
...
function treeLines(element, depth, label, body) {
    ...
    if (children.length === 0)
        return [`${indent}${label(elementLabel(element))}`];
    ...
    for (const child of children) {
        if (typeof child === 'string')
            lines.push(...textBlock(child, depth + 1, body));
        else
            lines.push(...treeLines(child, depth + 1, label, body));
    }
    return lines;
}
/**
 ...
 */
export function preview(lines, limit, omitted) {
 ...
 */
export function renderUnknownXml(source, maxChildLines, expanded, display, label, body, omitted) {
    ...
    if (expanded)
        return [rootLine, ...blocks.flat()];
    const previewed = blocks.map(block => preview(block, maxChildLines, omitted));
    if (previewed.length <= maxChildLines)
        return [rootLine, ...previewed.flat()];
...
//# sourceMappingURL=xml-tool-output.js.map