...
function runningPhaseGlyph(events, running, compacting) {
	if (running) return TIMING_BUCKET_GLYPHS[openStepPhase(events) ?? "ttft"];
	...
	*/
	...
	render(width) {
		...
		const art = DEEPSEEK_ART_5.flatMap((line, index) => {
			...
			const painted = index === DEEPSEEK_ART_5.length - 1 ? `${clipped} ${this.gradient ? gradientText("HARNESS") : this.palette.bold(this.palette.code("HARNESS"))}` : clipped;
		...
		].flatMap((line, index) => index < art.length ? [line] : wrapTextWithAnsi(line, usable)).map((line) => ` ${truncateToWidth(line, usable, "")}`);
...
var ToolCardComponent = class extends CachedCardComponent {
	...
	*/
	...
	renderLines(width) {
		...
		const lines = ["", statusColor(truncateToWidth(`${glyph} Tool / ${displayText(this.name)}${desc === void 0 ? "" : ` / ${displayInlineText(desc)}`}`, Math.max(1, width - 2), ""))];
...
var ContextCardComponent = class extends CachedCardComponent {
	...
	*/
	...
	renderLines(width) {
		...
		return [header, ...new Text((this.expanded ? body : preview(body, this.maxOutputLines, (count) => this.palette.dim(`… +${count} lines (Ctrl+O to expand)`))).join("\n"), 0, 0).render(width)];
	}
};
	...
	render(width) {
		...
		for (const [groupIndex, group] of this.groups.entries()) {
			...
			for (const [label, value] of group) {
				...
				const wrapped = wrapTextWithAnsi(value, Math.max(1, innerWidth - visibleWidth(prefix)));
...
var ModelDialog = class {
	...
	handleInput(data) {
		...
		else if (matchesKey(data, Key.escape)) if (this.filter.getValue() === "") this.cancel();
		else {
...
var ResumePicker = class {
	...
	*/
	...
	handleInput(data) {
		...
		if (matchesKey(data, Key.escape)) if (this.search.getValue() === "") this.cancel();
		else {
	...
	*/
	...
	render(width) {
		...
		const clippedSearch = truncateToWidth(this.search.render(searchInnerWidth).join("").replace(/^> /u, "⌕ "), searchInnerWidth, "");
...
var QuestionDialog = class {
	...
	handleInput(data) {
		...
		} else if (matchesKey(data, Key.space) && this.question.multiSelect) if (this.selected.has(this.selectedIndex)) this.selected.delete(this.selectedIndex);
...
function formatCwd(cwd) {
	...
	const rel = relative(resolve(homedir()), resolve(cwd));
	if (rel === "") return "~";
	...
	*/
	...
	const handoffResume = async (candidate, overlay) => {
		...
		try {
			...
			if (!deps.isDisposed()) if (terminalReleased) {
				ui.start();
				ui.setFocus(editor);
