 ...
 */
export function createVscodePalette(enabled: boolean): Palette {
  ...
  const roles = {} as Record<string, unknown>
  for (const name of COLOR_ROLES) roles[name] = ansi(spec.colors[name as typeof COLOR_ROLES[number]], enabled)
  for (const name of ATTRIBUTE_ROLES) roles[name] = ansi(spec.attributes[name as typeof ATTRIBUTE_ROLES[number]], enabled)
  return roles as unknown as Palette
}
