...
import { brandText, createPalette, createVscodePalette, markdownTheme, renderPalette, selectTheme } from './components/theme.ts'
...
function lastSessionActivityTime(events: readonly SessionEvent[]): number | undefined {
  let latest: number | undefined
  for (const event of events) {
    if (latest === undefined || event.time > latest) latest = event.time
  }
  return latest
}
