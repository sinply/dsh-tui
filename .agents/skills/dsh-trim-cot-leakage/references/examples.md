...
Distilled from a repo-wide leakage purge and its review rounds. Use them to identify the governing principle, not as text templates. This file deliberately quotes leaked wording as calibration material — the [recall batteries](recall-batteries.md) exclude the skill's directory, and its wording is not a license elsewhere.
...
**Fixed:** "Slash input resolves against the visible catalog — the plain-text-reference decision, documented in the README's input-resolution section."

The ordinal resolves nowhere at HEAD; the decision's name and owning document do. Name the owning document's path at least once per file — as a link where the surface supports one — and later mentions may use the searchable name alone.
...
**Leaked:** "Layering follows the design (v2 §3.2): `src/chat/` is the pure input core."

**Fixed:** "Layering: `src/chat/` is the pure input core."
...
**Leaked:** "`src/chat/` is the shell (T4); the P-I migration owns the prompts."

**Fixed:** "`src/chat/` is the shell; the prompts live in `src/prompt.ts`."
...
Both live facts survive — the current mechanism and the standing failure behavior — restated in the present. The bug's biography belongs to the PR thread, not to durable prose.
...
"This cut" / "v1" / "today" go stale the moment they merge. A historical stage name inside a section that owns its change story (a postmortem, a design doc) is current-state-safe; the indexical form never is.
...
A committed document states shipped reality; its own revision history lives in git.
...
An unaided pass deleted this, reasoning that issue citations need a special archive. Wrong direction: issues resolve at HEAD from any surface, and "#N owns the follow-up" is the sanctioned home for deferred work in a README. Citing *merged PRs* as evidence is likewise permitted wherever a surface calls for it — the ban is vantage, not citation.
...
**Original:** "The paragraph narrates the sync order; the same paragraph is what the Chinese README mirrors paragraph-for-paragraph."

**Overcorrected:** "…" (whole sentence deleted as narration.)

**Right:** "The Chinese README mirrors this paragraph paragraph-for-paragraph."
...
**Original:** "The 512-deep cap is measured: the largest observed payload nests 480 levels."

**Overcorrected:** "The cap is 512 nests; the largest observed payload nests 480 levels."

**Right:** keep "measured".

Without "measured" the 480 reads as a definition rather than an observation, and nobody re-measures before raising the cap.